...
if [ ${create_success_count} -ne ${expected_count} ]; then
  ...
  for key in ${keys}; do
    ...
    if [ "${key_create_success_count}" -ne "${RFPLUSONE}" ]; then
      CREATE_TEST_STATUS="${FAILURE}"
      break
    fi
  done
...
if [ "${CREATE_TEST_STATUS}" -eq "${SUCCESS}" ]; then
  CREATE_TEST_SCORE=3
fi
...
if [ "${delete_success_count}" -ne "${expected_count}" ]; then
  ...
  for key in ${keys}; do
    if [ $key != "${INVALID_KEY}" ]; then
      ...
      if [ "${key_delete_success_count}" -ne "${RFPLUSONE}" ]; then
        DELETE_TEST1_STATUS="${FAILURE}"
        break
      fi
    fi
...
if [ "${delete_fail_count}" -ne 4 ]; then
  DELETE_TEST2_STATUS="${FAILURE}"
fi

if [ "${DELETE_TEST1_STATUS}" -eq "${SUCCESS}" ]; then
  DELETE_TEST1_SCORE=3
fi

if [ "${DELETE_TEST2_STATUS}" -eq "${SUCCESS}" ]; then
...
cnt=1
for time in ${read_operations}; do
  if [ ${cnt} -eq 1 ]; then
...
if [ "${read_successes}" ]; then
  while read success; do
    time_of_this_success=$(echo "${success}" | cut -d" " -f2 | tr -s '[' ' ' | tr -s ']' ' ')
    if [ "${time_of_this_success}" -ge "${read_op_test1_time}" -a "${time_of_this_success}" -lt "${read_op_test2_time}" ]; then
      read_test1_success_count=$(expr ${read_test1_success_count} + 1)
    elif [ "${time_of_this_success}" -ge "${read_op_test2_time}" -a "${time_of_this_success}" -lt "${read_op_test3_part1_time}" ]; then
      read_test2_success_count=$(expr ${read_test2_success_count} + 1)
    elif [ "${time_of_this_success}" -ge "${read_op_test3_part2_time}" -a "${time_of_this_success}" -lt "${read_op_test4_time}" ]; then
      read_test3_part2_success_count=$(expr ${read_test3_part2_success_count} + 1)
    elif [ "${time_of_this_success}" -ge "${read_op_test4_time}" ]; then
...
if [ "${read_fails}" ]; then
  while read fail; do
    ...
    if [ "${time_of_this_fail}" -ge "${read_op_test3_part1_time}" -a "${time_of_this_fail}" -lt "${read_op_test3_part2_time}" ]; then
      actual_key=$(echo "${fail}" | grep "${read_op_test3_part1_key}" | wc -l)
      if [ "${actual_key}" -eq 1 ]; then
        read_test3_part1_fail_count=$(expr ${read_test3_part1_fail_count} + 1)
      fi
    elif [ "${time_of_this_fail}" -ge "${read_op_test5_time}" ]; then
...
if [ "${read_test1_success_count}" -eq "${QUORUMPLUSONE}" -o "${read_test1_success_count}" -eq "${RFPLUSONE}" ]; then
  READ_TEST1_STATUS="${SUCCESS}"
fi
if [ "${read_test2_success_count}" -eq "${QUORUMPLUSONE}" ]; then
  READ_TEST2_STATUS="${SUCCESS}"
fi
if [ "${read_test3_part1_fail_count}" -eq 1 ]; then
  READ_TEST3_PART1_STATUS="${SUCCESS}"
fi
if [ "${read_test3_part2_success_count}" -eq "${QUORUMPLUSONE}" -o "${read_test3_part2_success_count}" -eq "${RFPLUSONE}" ]; then
  READ_TEST3_PART2_STATUS="${SUCCESS}"
fi
if [ "${read_test4_success_count}" -eq "${QUORUMPLUSONE}" -o "${read_test4_success_count}" -eq "${RFPLUSONE}" ]; then
  READ_TEST4_STATUS="${SUCCESS}"
fi
if [ "${read_test5_fail_count}" -eq "${QUORUMPLUSONE}" -o "${read_test5_fail_count}" -eq "${RFPLUSONE}" ]; then
  READ_TEST5_STATUS="${SUCCESS}"
fi

if [ "${READ_TEST1_STATUS}" -eq "${SUCCESS}" ]; then
  READ_TEST1_SCORE=3
fi
if [ "${READ_TEST2_STATUS}" -eq "${SUCCESS}" ]; then
  READ_TEST2_SCORE=9
fi
if [ "${READ_TEST3_PART1_STATUS}" -eq "${SUCCESS}" ]; then
  READ_TEST3_PART1_SCORE=9
fi
if [ "${READ_TEST3_PART2_STATUS}" -eq "${SUCCESS}" ]; then
  READ_TEST3_PART2_SCORE=10
fi
if [ "${READ_TEST4_STATUS}" -eq "${SUCCESS}" ]; then
  READ_TEST4_SCORE=6
fi
if [ "${READ_TEST5_STATUS}" -eq "${SUCCESS}" ]; then
  READ_TEST5_SCORE=3
fi
...
cnt=1
for time in ${update_operations}; do
  if [ ${cnt} -eq 1 ]; then
...
if [ "${update_successes}" ]; then
  while read success; do
    time_of_this_success=$(echo "${success}" | cut -d" " -f2 | tr -s '[' ' ' | tr -s ']' ' ')
    if [ "${time_of_this_success}" -ge "${update_op_test1_time}" -a "${time_of_this_success}" -lt "${update_op_test2_time}" ]; then
      update_test1_success_count=$(expr ${update_test1_success_count} + 1)
    elif [ "${time_of_this_success}" -ge "${update_op_test2_time}" -a "${time_of_this_success}" -lt "${update_op_test3_part1_time}" ]; then
      update_test2_success_count=$(expr ${update_test2_success_count} + 1)
    elif [ "${time_of_this_success}" -ge "${update_op_test3_part2_time}" -a "${time_of_this_success}" -lt "${update_op_test4_time}" ]; then
      update_test3_part2_success_count=$(expr ${update_test3_part2_success_count} + 1)
    elif [ "${time_of_this_success}" -ge "${update_op_test4_time}" ]; then
...
if [ "${update_fails}" ]; then
  while read fail; do
    time_of_this_fail=$(echo "${fail}" | cut -d" " -f2 | tr -s '[' ' ' | tr -s ']' ' ')
    if [ "${time_of_this_fail}" -ge "${update_op_test3_part1_time}" -a "${time_of_this_fail}" -lt "${update_op_test3_part2_time}" ]; then
    ...
    elif [ "${time_of_this_fail}" -ge "${update_op_test5_time}" ]; then
      actual_key=$(echo "${fail}" | grep "${INVALID_KEY}" | wc -l)
      if [ "${actual_key}" -eq 1 ]; then
...
if [ "${update_test1_success_count}" -eq "${QUORUMPLUSONE}" -o "${update_test1_success_count}" -eq "${RFPLUSONE}" ]; then
  UPDATE_TEST1_STATUS="${SUCCESS}"
fi
if [ "${update_test2_success_count}" -eq "${QUORUMPLUSONE}" ]; then
  UPDATE_TEST2_STATUS="${SUCCESS}"
fi
if [ "${update_test3_part1_fail_count}" -eq 1 ]; then
  UPDATE_TEST3_PART1_STATUS="${SUCCESS}"
fi
if [ "${update_test3_part2_success_count}" -eq "${QUORUMPLUSONE}" -o "${update_test3_part2_success_count}" -eq "${RFPLUSONE}" ]; then
  UPDATE_TEST3_PART2_STATUS="${SUCCESS}"
fi
if [ "${update_test4_success_count}" -eq "${QUORUMPLUSONE}" -o "${update_test4_success_count}" -eq "${RFPLUSONE}" ]; then
  UPDATE_TEST4_STATUS="${SUCCESS}"
fi
if [ "${update_test5_fail_count}" -eq "${QUORUMPLUSONE}" -o "${update_test5_fail_count}" -eq "${RFPLUSONE}" ]; then
  UPDATE_TEST5_STATUS="${SUCCESS}"
fi

if [ "${UPDATE_TEST1_STATUS}" -eq "${SUCCESS}" ]; then
  UPDATE_TEST1_SCORE=3
fi
if [ "${UPDATE_TEST2_STATUS}" -eq "${SUCCESS}" ]; then
  UPDATE_TEST2_SCORE=9
fi
if [ "${UPDATE_TEST3_PART1_STATUS}" -eq "${SUCCESS}" ]; then
  UPDATE_TEST3_PART1_SCORE=9
fi
if [ "${UPDATE_TEST3_PART2_STATUS}" -eq "${SUCCESS}" ]; then
  UPDATE_TEST3_PART2_SCORE=10
fi
if [ "${UPDATE_TEST4_STATUS}" -eq "${SUCCESS}" ]; then
  UPDATE_TEST4_SCORE=6
fi
if [ "${UPDATE_TEST5_STATUS}" -eq "${SUCCESS}" ]; then
